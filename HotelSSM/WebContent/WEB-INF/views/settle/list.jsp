...
<div class="easyui-layout" data-options="fit:true">
    ...
    <div class="layui-col-xs6">
            ...
            <!--   <button type="button" class="layui-btn" onclick="remove()">删除</button> -->
               <button type="button" class="layui-btn" onclick="settle()">结账</button>
    </div>
    </div>
...
<script type="text/javascript">
	...
	*/
	...
	function settle(){
		...
		$.messager.confirm('信息提示','是否确认结账？', function(result){
			if(result){
				var item = $('#data-datagrid').datagrid('getSelected');
				if(item == null || item.length == 0){
					$.messager.alert('信息提示','请选择要删除的数据！','info');
				...
				$.ajax({
					...
					success:function(data){
						...
						if(data.type == 'success'){
							...
							$.messager.confirm('信息提示','是否确认结账？结账金额为：'+totalMoney, function(result){
								if(result){
									var item = $('#data-datagrid').datagrid('getSelected');
									if(item == null || item.length == 0){
										$.messager.alert('信息提示','请选择要删除的数据！','info');
