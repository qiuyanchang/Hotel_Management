...
<div id="section">
    ...
    <!--预定信息-->
    <div class="book_info" style="background:url(../resources/admin/login/images/bg3.jpg) top left;
background-size:100%;" >
        <form id="order_info" >
            <ul>
                ...
                <li>
                    ...
                    <div class="info_group" id="divId">
                        <label>入住时间</label><input type="text" name="arriveDate" id="arriveDate" class="datepicker"/>
						<label>离店时间</label><input type="text" name="leaveDate" id="leaveDate" class="datepicker" onchange="checkTime()" />
                    </div>
                    
                    <div class="info_group">
                        <label id="priceTotal">房费总计</label><span class="total">￥${roomType.price }</span>
                        <input  id="priceInput" type="hidden" value= "${roomType.price }" />
                    </div>
                      <div class="info_group">
                ...
                <li>
                    <div id="btn_booking" style="background:#000; color:#FFF;">确认预定</div>
					 <div class="jump">
         
                           <a href="../index"><span style="color:black;">返回列表</span></a>
                      </div>
                </li>
            </ul>
        </form>
...
function checkTime(){
	...
	var oDate1 = new Date(arriveDate);
    var oDate2 = new Date(leaveDate);

    if(oDate1.getTime() > oDate2.getTime()){
        alert('到达时间不能大于入住时间！');
    }else{
    	var between=(oDate2.getTime()-oDate1.getTime())/(1000*60*60*24);
   
        var onePrice=parseFloat($("#priceInput").val());
        var total=between*onePrice;
        $("#priceTotal").next("span.total").text("￥"+total+"");
    } 
}
